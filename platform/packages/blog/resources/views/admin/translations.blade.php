@php
    /** @var \Sitewyn\Packages\Blog\Models\Post $post */
    /** @var \Illuminate\Support\Collection<int, \Sitewyn\Core\Base\Models\Language> $languages */
    // Section "Translations" of the post form (P5-01): one card per active
    ...
    $existing = $post->exists ? $post->translations->keyBy('locale') : collect();
@endphp
  ...
  @foreach ($languages as $language)
    ...
    >
      ...
      <x-admin-editor
        ...
        label="Content"
        :height="360"
        :placeholder="$post->content !== null ? \Illuminate\Support\Str::limit(trim(strip_tags($post->content)), 120) : null"
      ...
      <div class="mb-0">
        ...
        <textarea
          ...
          rows="2"
          maxlength="500"
          placeholder="{{ $post->seo_description }}"
